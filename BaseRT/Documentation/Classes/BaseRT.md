# BaseRT
### The public entry point for managing a `BaseRT-server` instance.
> BaseRT.new (port : Integer; huggingfaces : cs.event.huggingfaces; HOME : 4D.Folder; options : Object; event : cs.event.event)
...
| options | Object | -> | Command-line options passed to `BaseRT-server` |
| event | cs.event.event | -> | Callback functions (onError, onSuccess, onData, onResponse, onTerminate) |

## Description

`cs.BaseRT.BaseRT` is the main class you instantiate to manage a `BaseRT-server` process. It extends `_interface` and orchestrates the full lifecycle: checking whether a server is already running on the given port, downloading model files from Hugging Face if needed, and starting the server process in the background via a worker.
...
The `options` object maps directly to `BaseRT-server` CLI flags. Underscores in property names are converted to hyphens (e.g. `n_gpu_layers` → `--n-gpu-layers`). Boolean `true` values produce flags without a value (e.g. `embeddings: True` → `--embeddings`). File and Folder objects are automatically path-escaped for the current platform.
...
var $BaseRT : cs.BaseRT.BaseRT
$BaseRT:=cs.BaseRT.BaseRT.new(8082; $huggingfaces; $homeFolder; $options; $event)
```
...
$iniFile:=$homeFolder.folder("BaseRT-8888").file("models.ini")
$iniFile.setText($ini.join("\n"))

$options:={embeddings: True; models_preset: $iniFile; n_gpu_layers: 99}
$BaseRT:=cs.BaseRT.BaseRT.new(8888; Null; $homeFolder; $options; $event)
```
...
- [`_server`](_server.md) — CLI wrapper for `BaseRT-server`
